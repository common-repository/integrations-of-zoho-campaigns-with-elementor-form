<?php
namespace FormInteg\IZCEF\Flow;

use FormInteg\IZCEF\Core\Util\Common;
use FormInteg\IZCEF\Core\Util\IpTool;
use FormInteg\IZCEF\Core\Util\SmartTags;
...
final class Flow
{
    ...
    {
        if (class_exists("FormInteg\\IZCEF\\Actions\\{$name}\\{$name}Controller")) {
            return "FormInteg\\IZCEF\\Actions\\{$name}\\{$name}Controller";
        } else {
            return false;
        }
    }
    ...
    {
        if (class_exists("FormInteg\\IZCEF\\Triggers\\{$name}\\{$name}Controller")) {
            return "FormInteg\\IZCEF\\Triggers\\{$name}\\{$name}Controller";
        } else {
            return false;
        }
    }
    ...
    public function get($data)
    {
        ...
        if (!is_null($missing_field)) {
            wp_send_json_error(sprintf(__('%s can\'t be empty', 'elementor-to-zoho-campaigns'), $missing_field));
        }
        $integrationHandler = new FlowController();
        $integrations = $integrationHandler->get(
            ['id' => $data->id],
    ...
    public function save($data)
    {
        ...
        wp_send_json_success(['id' => $saveStatus, 'msg' => __('Integration saved successfully', 'elementor-to-zoho-campaigns')]);
    }
    ...
    public function flowClone($data)
    {
        ...
        if (!is_null($missingId)) {
            wp_send_json_error(sprintf(__('%s can\'t be empty', 'elementor-to-zoho-campaigns'), $missingId));
        }
        $integrationHandler = new FlowController();
        $integrations = $integrationHandler->get(
            ['id' => $data->id],
        ...
        if (!is_wp_error($integrations) && count($integrations) > 0) {
            ...
            wp_send_json_error(__('Flow ID is not exists', 'elementor-to-zoho-campaigns'));
        }
    }
    ...
    public function update($data)
    {
        ...
        wp_send_json_success(__('Integration updated successfully', 'elementor-to-zoho-campaigns'));
    }
    ...
    public function delete($data)
    {
        ...
        if (!is_null($missing_field)) {
            wp_send_json_error(sprintf(__('%s cann\'t be empty', 'elementor-to-zoho-campaigns'), $missing_field));
        }
        $integrationHandler = new FlowController();
        ...
        wp_send_json_success(__('Integration deleted successfully', 'elementor-to-zoho-campaigns'));
    }

    public function bulkDelete($param)
    {
        if (!is_array($param->flowID) || $param->flowID === []) {
            wp_send_json_error(sprintf(__('%s cann\'t be empty', 'elementor-to-zoho-campaigns'), 'Integration id'));
        }
    ...
    public function toggle_status($data)
    {
        ...
        wp_send_json_success(__('Status changed successfully', 'elementor-to-zoho-campaigns'));
    }
        ...
        if (!is_wp_error($flows) && !empty($flows)) {
            ...
            foreach ($flows as $flowData) {
                ...
                if (
                    ...
                    $error = new WP_Error('Conditional Logic False', __('Conditional Logic not matched', 'elementor-to-zoho-campaigns'));
